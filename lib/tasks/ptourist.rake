namespace :ptourist do
  ...
  def create_image_content img
    ...
    original_content = ImageContent.new(image_id: img[:image].id,
        content_type: "image/jpeg", content: BSON::Binary.new(contents))
  ...
  def create_thing thing, organizer, members, images
    ...
    m = members.map { |member|
      unless member.id == organizer.id or member.id == mike_user.id
  ...
  task users: [:delete_all] do
    ...
    admin_users.each { |user| user.roles.create(role_name: Role::ADMIN) }
    originator_users.each { |user| user.add_role(Role::ORIGINATOR, Thing).save }
  ...
  task subjects: [:users] do
    ...
    members = boy_users
    images = [{ :path => "db/bta/image001_original.jpg",
    ...
    organizer = get_user("alice")
    members = boy_users
    images = [{ :path => "db/bta/DSC_5358.jpg",
    ...
    members = boy_users
    images = [{ :path => "db/bta/image004_original.jpg",
     :caption => "Overview",
     :lng => nil,
     :lat => nil },
    ...
    members = girl_users
    images = [{ :path => "db/bta/hitim-001.jpg",
...
"}
    ...
    members = girl_users
    images = [{ :path => "db/bta/hpm-001.jpg",
     :caption => "Hotel Front Entrance",
     :lng => -76.5987,
     :lat => 39.2847,
     :priority => 0 },
  ...
  def create_tag name, thing_ids
    ...
    thing_ids.each { |i| ThingTag.create(thing_id: i, tag_id: tag.id) }
  end
  ...
  task :tags => :environment do
    ...
    create_tag('Museum', [1, 2])
    create_tag('Restaurant', [3, 4])
    create_tag('Attraction', [5, 6])
  end
end
