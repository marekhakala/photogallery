class ThingImage < ActiveRecord::Base
  ...
  scope :with_thing, -> { joins("left outer join things on things.id = thing_images.thing_id").select("thing_images.*") }
  scope :with_image, -> { joins("right outer join images on images.id = thing_images.image_id").select("thing_images.*", "images.id as image_id") }
  scope :with_tag, -> (id) { joins("join thing_tags on thing_images.thing_id = thing_tags.thing_id")
    .select("thing_images.*", "thing_tags.tag_id").where("thing_tags.tag_id" => id) }
