  ...
  factory :thing do
    ...
    trait :with_roles do
      ...
      after(:create) do |thing, props|
        Role.create(role_name: Role::ORGANIZER, mname: Thing.name, mid: thing.id, user_id: props.originator_id)
        Role.create(role_name: Role::MEMBER, mname: Thing.name, mid: thing.id, user_id: props.member_id) if props.member_id
      end
    end
  end
end
