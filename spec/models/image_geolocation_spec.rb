...
RSpec.describe "Image Geolocation", type: :model do
  ...
  context "ThingImage geolocation thru Image" do
    ...
    before(:all) do
      (0..90).each do |idx|
        ...
        FactoryGirl.create(:thing_image, thing: thing, image: image, priority: idx % 2)
      end
    end
    ...
    it "finds ThingImage with near Images with distance" do
      near = ThingImage.select("thing_images.*").select("images.lat, images.lng").joins(:image).within(10*69, origin: origin)
    ...
    it "finds Thing near primary Image" do
      ...
      ThingImage.all.each { |ti| primary_in_range += 1 if (ti.priority == 0 && ti.image.distance_from(origin) <= 10*69) }
      expect(primary_in_range).to be <= ThingImage.count/2
      near = ThingImage.joins(:image).within(10*69, origin: origin).where(priority: 0)
  ...
  describe "ThingImage geo queries" do
    ...
    before(:all) do
      ...
      orphan_image = FactoryGirl.create(:image, caption: "orphan", image_content: nil, position: Point.new(0,1))

      (0..2).each do |idx|
        thing = FactoryGirl.create(:thing)

        (0..1).each do |priority|
          image = FactoryGirl.create(:image, caption: (priority == 0) ? "priority" : "secondary",
                                      image_content: nil, position: Point.new(0,idx))
          FactoryGirl.create(:thing_image, thing: thing, image: image, priority: priority)
        end
      end
    end

    it "supplies Image and Thing info" do
      result = ThingImage.where(id: sample.id).with_name.with_caption.with_position.first
    ...
    it "returns ThingImages for orphan Images" do
      orphan_images = ThingImage.within_range(origin).where(thing: nil).with_caption
    ...
    it "returns ThingImages within limit" do
      ...
      results.each { |ti| expect(ti.distance).to be < 70 }
    end
