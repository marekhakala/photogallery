module SubjectsUiHelper
  ...
  def image_editor_loaded! image, expected_linkables = nil
    ...
    expect(page).to have_css(".link-things select option", count: expected_linkables) if expected_linkables and logged_in?
  end

  def visit_image image
    unless page.has_css?("sd-image-editor .image-form span.image_id", text: image.id, visible: false)
  ...
  def populate_subjects
    ...
    (1..3).each do
      ...
      if image = shared_images.sample
        thing.thing_images.create(priority: 5, image_id: image.id, creator_id: user[:id])
      end
  ...
  def has_no_current_thing_id
    within("sd-area[label='Map']") do
      find("div.tabs-pane ul li a", text: "Things").click
      expect(page).to have_no_css("div.tabs-pane ul.things li.selected span.thing_id", visible: false)
    end
  end
  ...
  def get_current_image_id
    within("sd-area[label='Subjects']") do
      ...
      id = find("div.tabs-pane ul.images li.selected span.image_id", visible: false).text(:all)
      id.to_i if id
    end
  end
