module ApiHelper
  ...
  def signup registration, status = :ok
    ...
    payload = parsed_body
    registration.merge(id: payload["data"]["id"], uid: payload["data"]["uid"]) if response.ok?
  end
  ...
  def access_tokens?
    not response.headers["access-token"].nil? if response
  end
  ...
  def except_content original
    if original[:image_content] and original[:image_content][:content]
