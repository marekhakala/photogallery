...
RSpec.feature "AuthzThingImages", type: :feature, js: true do
  ...
  let(:things) { FactoryGirl.create_list(:thing, 3, :with_roles,
    originator_id: originator[:id], member_id: member[:id]) }
  let(:alt_things) { FactoryGirl.create_list(:thing, 1, :with_roles,
    originator_id: originator[:id], member_id: alt_member[:id]) }
  let(:images) { FactoryGirl.create_list(:image, 3, :with_roles,
    creator_id: authenticated[:id]) }

  let(:linked_thing) { things[0] }
  let(:linked_image) { images[0] }
  let(:thing_image) { FactoryGirl.create(:thing_image, thing: linked_thing,
    image: linked_image, priority: 3, creator_id: member[:id]) }
  ...
  shared_examples "can get links" do
      ...
      it "can view linked things for image" do
        within("sd-image-editor") do
          ...
          within("div.image-things") do
            ...
            expect(page).to have_css("li span.thing_id", text: linked_thing.id, visible: false)
            expect(page).to have_no_css("li span.thing_id")
          end
        end
    ...
    context "from things" do
      ...
      it "can view linked images for thing" do
        ...
        expect(page).to have_css(".image-area img[src*='#{image_content_path(linked_image)}']", visible: false, wait: 5)
      end
  ...
  shared_examples "can create links" do
    ...
    it "can create link image to things" do
      within("sd-image-editor .image-form") do
        ...
        expect(page).to have_css("ul.image-things li span.thing_id", text: things[1].id, visible: false)
      end
    end
    ...
    it "removes link button when no linkables" do
      ...
      within("sd-image-editor .image-form") do
        expect(page).to have_css(".link-things select option", count: linkables.size, wait: 5)
        all(".link-things select option").each { |option| option.select_option }

        button = page.has_button?("Update Image") ? "Update Image" : "Link to Things"
        click_button button

        expect(page).to have_button(button, disabled: true, wait: 5)
        expect(page).to have_css("ul.image-things li span.thing_id", text: things[1].id, visible: false, wait: 5)
      end
    end
  end
  ...
  shared_examples "cannot create link" do
    ...
    it "shows no linkable things for image" do
      within("sd-image-editor .image-form") do
        things.each { |thing| expect(page).to have_no_css("select option", text: thing.name) }
      end
    end
  end
  ...
  shared_examples "can edit link" do
    ...
    it "can adjust priority of image" do
      within("sd-thing-editor .thing-form") do
        ...
        expect(page).to have_css("div.thing-images span.image_id", text: linked_image.id, visible: false, wait: 5)

        image_li = find("div.thing-images span.image_id", text: linked_image.id, visible: false).find(:xpath, "../..")
  ...
  shared_examples "can remove link" do |role|
    ...
    it "can remove link to image" do
      ...
      within ("sd-thing-editor .thing-form") do
        ...
        expect(page).to have_no_css(".thing-images ul li", text: displayed_caption(linked_image))

        expect(ThingImage.where(id: linked_image.id)).to_not exist
        expect(page).to have_css(".thing-images ul li", count: ThingImage.where(thing: linked_thing).count)
