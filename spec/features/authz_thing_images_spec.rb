...
RSpec.feature "AuthzThingImages", type: :feature, js: true do
  ...
  let(:things) { FactoryGirl.create_list(:thing, 3, :with_roles, originator_id: originator[:id], member_id: member[:id]) }
  let(:alt_things) { FactoryGirl.create_list(:thing, 1, :with_roles, originator_id: originator[:id], member_id: alt_member[:id]) }
  let(:images) { FactoryGirl.create_list(:image, 3, :with_roles, creator_id: authenticated[:id]) }
  let(:linked_thing) { things[0] }
  let(:linked_image) { images[0] }
  let(:thing_image) { FactoryGirl.create(:thing_image, thing: linked_thing, image: linked_image,
     priority: 3, creator_id: member[:id]) }
  ...
  shared_examples "can get links" do
    context "from images" do
      ...
      it "can navigate from image to thing" do
        ...
        link_selector_args=["sd-image-editor ul.image-things span.thing_id", { text: linked_thing.id, visible: false, wait: 5 }]
    ...
    context "from things" do
      ...
      it "can view linked images for thing" do
        ...
        within("sd-thing-editor .thing-form ul.thing-images", wait: 5) do
          expect(page).to have_css("li span.image_id", text: linked_image.id, visible: false, wait: 5)
          expect(page).to have_css("li label.image-caption", text: linked_image.caption)
      ...
      it "can navigate from thing to image" do
        ...
        within("sd-thing-editor .thing-form ul.thing-images") do
          node = find("li span.image_id", text: linked_image.id, visible: false, wait: 5).find(:xpath, "../label").click
        end
